...
function Boss:create(data)
	...
	-- spawn drones = small versions of normal (mobile) enemies
	boss.spawnWait = 4
	boss.spawnTick = 0
	...
	function boss:tick(dt)
		...
		if self.spawnTick >= self.spawnWait then
			self:spawnDrone()
		end
	...
	-- spawn smaller enemies (3/4 size)
	function boss:spawnDrone()
		-- orig enemy size:
